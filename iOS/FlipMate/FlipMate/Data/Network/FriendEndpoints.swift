...
struct FriendEndpoints {
    ...
    static func unfollowFriend(with friendUnfollowRequestDTO: FriendUnfollowRequestDTO) -> EndPoint<StatusResponseDTO> {
        let encoder = JSONEncoder()
    ...
    static func getChartInfo(with socialDetailRequestDTO: SocialDetailRequestDTO) -> EndPoint<SocialDetailResponseDTO> {
        let encoder = JSONEncoder()
